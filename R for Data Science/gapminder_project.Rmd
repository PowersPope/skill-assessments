...
# function to generate the correlation between all years
correlation.group <- function(df){
  return(data.frame(COR = cor.test(df$`CO2 emissions (metric tons per capita)`, df$gdpPercap)$estimate[[1]]))
}

# calculate the amount of correlation for each year
correlation.year <- ddply(data, .(Year),  correlation.group)
# grab the highest correlated year
highest.year <- correlation.year[order(correlation.year$COR, decreasing=TRUE),][1,'Year']

# Subset the data
high.corr <- filter(data, Year == highest.year)
```
...
index <- 0
for (country in continent.df$name) {
  index <- index + 1
  sub.x <- which(data$`Country Name` == country)
  len.sub.x <- length(sub.x)
  if (len.sub.x > 0){
    data[sub.x,'continent'] <- continent.df[index, 'region']
  }
}

```

Once we bring in the other data frame and use it to fill in `continent` we get `r sum(is.na(data$continent))`
`NA` values.
...
While also getting rid of the NA rows within `continent`, so they don't confuse the
analysis.
...
```{r subset Population}
# Grab the top population density countries over the years
grab_max  <- function (df) {
  top <- df %>% filter(`Population density (people per sq. km of land area)` == max(na.omit(`Population density (people per sq. km of land area)`)))
  x <- data.frame(max.country=top$`Country Name`,pop=top$`Population density (people per sq. km of land area)`)
  return(x)
}
# top countries and number
avg_country <- ddply(data, .(Year), grab_max)

#plotly graph 
ggplotly(ggplot(avg_country, aes(x=Year, y=pop, fill=max.country))+
...
```{r question 4 plot}
# Function to grab the difference between the 2007 - 1962 Life Expectancy
diff_life <- function (df) {
  unique_names <- unique(df$`Country Name`)
  x <- data.frame(country=unique_names, year_range=0)
  year.1962 <- df[df$Year == 1962,]
  year.2007 <- df[df$Year == 2007,]
  count = 0
  for (name in unique_names) {
    count = count + 1
    true.2007 <- name %in% year.2007$`Country Name`
    true.1962 <- name %in% year.1962$`Country Name`
    if (all(true.2007 & true.1962)) {
      diff_nums <- year.2007[year.2007$`Country Name` == name,]$`Life expectancy at birth, total (years)` -
        year.1962[year.1962$`Country Name` == name,]$`Life expectancy at birth, total (years)`
      x[count,]$year_range <- diff_nums
    }
  }
  return(x)
}
...
# order data
order.total <- y[order(y$year_range, decreasing=TRUE),]
top.five <- order.total[1:5,]

# Data
ggplotly(ggplot(y, aes(x=reorder(country,year_range), y=year_range))+
  geom_bar(stat="identity")+
  labs(title="Country Life Expectancy Increase from 1962-2007",
       x="Countries", y="Increase of Life Expectancy")+
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()))

knitr::kable(top.five)
```
...
top.full.five <- data %>% filter(`Country Name` %in% c("Maldives", "Bhutan",
                                                       "Timor-Leste", "Tunisia", "Oman", "Nepal"))

ggplotly(ggplot(top.full.five, aes(x=Year, y=`Life expectancy at birth, total (years)`,
